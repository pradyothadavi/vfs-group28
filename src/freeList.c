...
struct freeList *s_insertFreeListNode(struct freeList *headPtr,struct freeList *tempNode){
    ...
    if(NULL == headPtr){
         ...
         if( NULL != tempNode ){
              ...
              tempNode->sPtr_nextFreeBlock = currentNode->sPtr_nextFreeBlock;
...
/*
Function Name: v_deleteList
Description: It free's the memory held by the free List.
Parameters: It takes the head pointer of the list to be freed as a parameter
Return Type: void
*/
void v_deleteList(struct freeList *headPtr){
    ...
    if( NULL == headPtr ){
         ...
         while( NULL != headPtr->sPtr_nextFreeBlock ){
              currentNode = headPtr;
              headPtr = headPtr->sPtr_nextFreeBlock;
              currentNode->sPtr_nextFreeBlock = NULL;
              free(currentNode);
         }
         free(headPtr);
