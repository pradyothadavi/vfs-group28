...
void v_deleteBSTNode(struct binarySearchTree *sPtr_root,
    ...
    sPtr_nodeToBeDeleted->parent = NULL;
    sPtr_nodeToBeDeleted->leftChild = NULL;
    sPtr_nodeToBeDeleted->rightChild = NULL;
    if(NULL != sPtr_nodeToBeDeleted ){
         free(sPtr_nodeToBeDeleted);
    }
}
...
Return Type:
*/
void v_deleteBST(struct binarySearchTree *sPtr_subTreeRoot){

    struct binarySearchTree *sPtr_nodeToBeDeleted = NULL;

    sPtr_nodeToBeDeleted = sPtr_subTreeRoot;

    if( NULL == sPtr_nodeToBeDeleted ){
         return;
    } else {
         v_deleteBST(sPtr_nodeToBeDeleted->leftChild);
         v_deleteBST(sPtr_nodeToBeDeleted->rightChild);
         if( sPtr_nodeToBeDeleted == sPtr_rootBST){
              sPtr_nodeToBeDeleted->parent = NULL;
              sPtr_nodeToBeDeleted->leftChild = NULL;
              sPtr_nodeToBeDeleted->rightChild = NULL;
              if(NULL != sPtr_nodeToBeDeleted ){
              ...
              v_deleteBSTNode(sPtr_rootBST,sPtr_nodeToBeDeleted);
         }
    }
}
/*
...
void v_storeDirectoryEntries(struct binarySearchTree *sPtr_toANode,int mode){
    ...
    cPtr_filePath = (char *)
                    malloc(sizeof(char)*strlen(sPtr_toANode->cArr_filePath));
    memset((void *)cPtr_filePath,'\0',
                           sizeof(char)*strlen(sPtr_toANode->cArr_filePath));
    ...
    if( (currentNode->s_inode->c_fileType[0] == 'd') &&
        (currentNode->s_inode->ui_inodeNo == sPtr_toANode->ui_inodeNo) ){
         ...
         if( UNMOUNTVERIFICATION == mode ){
              currentNode = s_searchNAryTreeNode(currentNode,NULL,
                                                 UNMOUNTVFSVERIFICATION);
         }
    }
